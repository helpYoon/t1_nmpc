...
"""Crocoddyl cost builders for the T1 whole-body MPC stand (M0) problem.

All terms map t1_controller costs to NATIVE crocoddyl residuals reading config_wb. The stance
contact wrench is bounded with a combined WrenchCone + QuadraticBarrier. (The walking problem is
built separately by croco_walk.WalkOCP, which uses the same QuadraticBarrier shape on split
FrictionCone + CoP residuals; `_control_weights` is shared.)
"""
...
def build_costs(state, actuation, nu, x_ref, com_ref, stance_fids, cfg, terminal=False):
    ...
    # --- 2. CoM tracking (M0: com_ref = com0, low weight; forward drive is the walk problem) ---
